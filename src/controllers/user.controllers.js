...
const registerUser = asyncHandler(async (req, res) => {
    ...
    let coverImage;
    try {
        coverImage = coverLocalPath ? await uploadOnCloudinary(coverLocalPath) : "";
        console.log("Uploaded coverImage", coverImage);
    } catch (error) {
        console.log("Error uploading coverImage", error);
        throw new ApiError(500, "Failed to upload coverImage");
...
const generateAccessAndRefreshToken = async (userId) => {
    try {
        ...
        if (!user) {
            throw new ApiError(404, "User not found"); // ✅ Existence check
        }
...
const loginUser = asyncHandler(async (req, res) => {
    // get data from body
    const {uemail, sername, password} = req.body
    // validation
    if(!email){
...
const loginUser = asyncHandler(async (req, res) => {
    const { email, username, password } = req.body; // ✅ fixed variable names
    if (!email && !username) {
    ...
    if(!loggedInUser) {
        throw new ApiError(500, "Login failed, user not found after token generation");
    }
    const options = {
        httpOnly: true,
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
    ...
    if(!isPasswordValid) {
        throw new ApiError(401, "Old passsword is incorrect");
    }
